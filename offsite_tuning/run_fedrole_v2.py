...
def train_global_harmonizers(full_model, tokenizer, accelerator, num_steps=200):
    logger.info("⚡ [Server] Pre-training Global Harmonizer Pool (Stochastic Swap)...")
...
def parse_args():
    ...
    parser.add_argument("--alpha", type=float, default=0.1) # not strictly needed for FedRole but kept for arg compat
...
def main():
    ...
    for cid in range(args.num_clusters):
        ...
        # --- FedRole: Block-wise Sensitivity Selection ---
    ...
    for cid, c_clients in clusters.items():
        ...
        # 传入 full_eval=True 开启全量测试，以获取最精确稳定的定稿 ACC
